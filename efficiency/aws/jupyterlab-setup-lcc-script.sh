...
# We use --no-check-certificate with wget to workaround the error below:
# 
...
wget https://raw.githubusercontent.com/aws-samples/sagemaker-studio-apps-lifecycle-config-examples/main/jupyterlab/auto-stop-idle/on-start.sh --no-check-certificate
bash on-start.sh
...
# to the location of the socket, but if SageMaker evolves, you can set that
